  ...
  try {
    ...
    try {
      ...
      if (e.message === 'SERVICE_UNAVAILABLE') {
        return { success: false, error: 'Le service de signature est momentanément indisponible (Configuration Admin manquante).' }
      }
      throw e
    }
}
...
export async function trackQuoteViewAction(quoteId: string, publicToken: string) {
  ...
  try {
    ...
    if (err.message === 'SERVICE_UNAVAILABLE') {
      console.warn('[trackQuoteViewAction] Admin Client unavailable, skipping tracking.')
    } else {
